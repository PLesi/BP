...
from .services import get_task_device_id, calculate_estimated_wait_time
...
def device_worker(device_id: int):
    ...
    if not queued_task:
        print(f"⚠️  Queue empty for device {device_id}")
        return
...
async def update_queue_positions(device_id: int):
    ...
    for task_json in redis_client.lrange(queue_key, 0, -1):
        ...
        estimated_wait_and_position_json = json.dumps(estimated_wait_and_position)
        await ws_manager.send_message(task_id, estimated_wait_and_position_json)
